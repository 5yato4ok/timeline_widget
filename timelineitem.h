...
public:
    ...
    TimeLineItem(int start, int end, const std::vector<int> &idxs,int p_y =0, double scale = 0) :
        start_sec(start),end_sec(end),bkmrks_idxs(idxs), y(p_y), hour_scale(scale){}
    TimeLineItem(const std::pair<int,int>& range, const std::vector<int> &idxs):TimeLineItem(range.first, range.second, idxs) {
    }
